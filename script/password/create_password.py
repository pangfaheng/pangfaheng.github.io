...
# use:
#   python $0 [option: int[password_lenght] int[password_number]]
#
# example:
...
def generate_character():
    ...
    for i in (
        ...
        if (
            i not in "!@$%^&*"
            and i not in string.ascii_lowercase + string.ascii_uppercase + string.digits
        ):
            continue
...
def check_password_strength(password):
    ...
    if (
        remark_lowercase == True
        and remark_luppercase == True
        and remark_digit == True
        and remark_punctuation == True
    ):
        return True
    else:
        return False
